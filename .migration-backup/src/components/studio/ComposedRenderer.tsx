      ...
      >
        {items.map((item) =>
          captions ? (
            ...
            >
              {art(item, size)}
              <span style={{ fontSize: `${LABEL_MM}mm`, color: p.inkSoft }}>
                {item.label ?? ""}
              </span>
            </span>
          ) : (
            ...
            {Array.from({ length: component.slots }).map((_, index) => (
              <div
                ...
                style={{
                  flex: 1,
                  height: "16mm",
                  borderBottom: `0.5mm solid ${p.rule}`,
                  borderTop: `0.3mm dashed ${p.rule}`,
                }}
              />
            ))}
          </div>
      ...
      case "cut-out-strip":
        return (
          ...
          >
            ...
            {component.note ? (
              <span style={{ fontSize: `${LABEL_MM}mm`, color: p.inkSoft }}>{component.note}</span>
            ) : null}
          </div>
        );
