...
const directiveRules: Rule[] = [
  ...
  { mechanic: "sort-attribute", test: /\bsort(s|ing)?\b|\bgroup(s|ing)?\b|\bclassif/i },
  {
    ...
    test: /\bcircl(e|es|ing) (the )?(number|numeral|answer)\b|\bhow many\b|\bcount(s|ing)?\b/i,
  },
];
...
export function semanticRequirementsOf(
  ...
  const choiceCount = countBeforeNoun(text, /(choices?|options?|pictures? per row)/);
...
function studentActionOf(
  ...
  if (/\bsort|\bgroup(s|ing)?\b|\bclassif/i.test(text)) return "sort";
...
export function parsePageDirectives(spec: WorksheetSpec): PageDirective[] {
  ...
  if (!lines.length) {
    ...
    return parseMechanicList(prompt).map((entry, index) => {
      const mechanic = listMath ? domainFallback(false, entry.mechanic) : entry.mechanic;
      return {
        page: index + 1,
        text: entry.text,
        mechanic,
        semanticRequirements: semanticRequirementsOf(entry.text, mechanic),
      };
    });
  }
  ...
  for (const line of lines) {
    const mechanic = mechanicForDirectiveText(line.text, literacy);
    ...
    directives.push({
      ...
      text: line.text,
      mechanic,
      semanticRequirements: semanticRequirementsOf(line.text, mechanic),
...
function quantityIssues(
  ...
  if (requiredChoiceCount && activity.kind === "pick-one") {
    const wrong = activity.rows.filter((row) => row.options.length !== requiredChoiceCount);
    if (wrong.length) {
      add(
