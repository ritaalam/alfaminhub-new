...
export function buildMemoryPairsPage(ctx: {
  ...
  const subject = resolveSubject(spec);
  const assets = vocabularyForSpec(spec, pairsForAge(spec.level));
  ...
  const activity: MemoryPairsActivity = {
    kind: "memory-pairs",
    mechanic: "memory-pairs",
    cards: shuffle(cards, next),
    showLabels: ageId !== "toddler-2-3",
    challenge: `Can you say the name of every picture as you turn it over?`,
  };
  ...
  return {
    ...
    activity,
    answerKey: assets.map((_asset, i) => ({
      groupId: `pair-${i + 1}`,
      answer: 2,
      answerText: titleCase(visualObjects[assets[i]!].singular),
    })),
