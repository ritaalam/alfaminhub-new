...
export function assetsForSpec(spec: WorksheetSpec): VisualAssetKey[] {
  return resolveSubject(spec).assets;
}
...
export function rangeForSpec(spec: WorksheetSpec): [number, number] {
  ...
  const [min, max] = difficultyRange(spec.difficulty);
...
export function buildCountMatchPage(spec: WorksheetSpec, seed: number): WorksheetPageModel {
  const next = rng(seed);
  const tokens = resolveAgeTokens(spec.level);
  ...
  const counts = uniqueCounts(tokens.itemsPerPage, range, next);
...
export function buildCountCirclePage(spec: WorksheetSpec, seed: number): WorksheetPageModel {
  ...
  const counts = uniqueCounts(Math.max(3, tokens.itemsPerPage - 2), range, next);
  ...
  const rows = counts.map((count, i) => {
    ...
    return {
      ...
      choices: choicesFor(count, range, next, tokens.answerChoices),
    };
  });
...
function buildPlannedPage(
  ...
  if (mechanic === "count-match") return assertPlanned(mechanic, buildCountMatchPage(spec, seed));
  ...
  if (mechanic === "count-circle") return assertPlanned(mechanic, buildCountCirclePage(spec, seed));
 ...
 */
function simplifyForAge(spec: WorksheetSpec, page: WorksheetPageModel): WorksheetPageModel {
  if (resolveAgeTokens(spec.level).id !== "toddler-2-3") return page;
  const activity = { ...page.activity } as WorksheetPageModel["activity"];
  if ("challenge" in activity) delete (activity as { challenge?: string }).challenge;
  const simplified: WorksheetPageModel = { ...page, activity };
  delete simplified.footerNote;
  return simplified;
}
...
export function buildWorksheetProject(
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    for (const issue of issues) {
      ...
      pages[index] = { ...simplifyForAge(spec, rebuilt), id: `page-${index + 1}` };
    }
  }
