...
import { normalizeWorksheetSpecForRenderer } from "@/lib/worksheet-renderer-support";
import {
  advancedActivityTypeSupportFor,
  promptActivityTypeFor,
  promptActivityTypeSupportFor,
  UnsupportedAdvancedActivityTypeError,
  withoutAdvancedActivityType,
} from "@/lib/worksheet-renderer-support";
...
export function WorksheetCreator({
  ...
  function persist(nextProject: WorksheetProject, nextSpec = spec) {
    ...
    try {
      ...
      setSavedDraftId(draft.id);
      // A saved worksheet is resumable by its draft id. Keep that identity in
      // the route so a browser reload enters the existing restore path instead
      // of intentionally creating a fresh default worksheet.
      if (new URLSearchParams(window.location.search).get("draft") !== draft.id) {
        void navigate({ to: "/", search: { draft: draft.id }, replace: true });
      }
      setSaveState("saved");
      setLastSavedAt(draft.updatedAt);
      setSaveError(null);
  ...
  const previewSpec = useMemo(
    () =>
      mode === "quick"
        ? canonicalQuickCreateRequest(spec)
        : normalizeWorksheetSpecForRenderer(applyPromptIntent(spec)),
    [mode, spec],
  );
  ...
  const set = (key: WorksheetField | "prompt", value: string) => {
    if (key === "prompt" && mode === "quick") {
      // Replacing a Quick Create prompt begins a new request. Update the
      // canonical spec immediately so the preview, planning snapshot and
      // generated Studio project share the same activity type.
      clearQuickCreateResult();
      setSpec((prev) => replaceQuickCreatePrompt(withoutAdvancedActivityType(prev), value));
      return;
    }
  ...
  async function handleGenerate() {
    ...
    if (loading) return;
    // React state updates are asynchronous. This ref closes the tiny window
    // where two rapid clicks can enter this handler before `loading` rerenders.
    if (generationInFlightRef.current) return;
    const advancedActivityType =
      mode === "advanced" && spec.source !== "idea-lab" ? spec.activityType : undefined;
    // An Advanced constraint belongs to that one Advanced request only. A
    // teacher returning to Quick Create must get Quick's usual open planning,
    // even if the previous worksheet was generated in Advanced mode.
    const requestSpec =
      mode === "quick"
        ? canonicalQuickCreateRequest(spec)
        : advancedActivityType
          ? { ...spec, advancedActivityType }
          : spec;
    const advancedSupport = advancedActivityType
      ? advancedActivityTypeSupportFor(advancedActivityType)
      : undefined;
    if (advancedSupport?.status === "unsupported") {
      setGenerationError(advancedSupport.message ?? `“${advancedActivityType}” is not available yet.`);
      return;
    }
    if (mode === "quick") {
      const promptActivityType = promptActivityTypeFor(requestSpec);
      const promptSupport = promptActivityTypeSupportFor(requestSpec);
      if (promptActivityType && (!promptSupport || promptSupport.status === "unsupported")) {
        setGenerationError(
          promptSupport?.message ??
            `“${promptActivityType}” is not available yet because Alfa does not have a matching printable renderer.`,
        );
        return;
      }
    }
    ...
    try {
      const planning = await planWorksheetSpec(
        requestSpec,
        mode,
        undefined,
        crypto.randomUUID(),
      ...
      // The planner may enrich content, but it cannot replace the current
      // Quick Create prompt's named activity. Re-derive the canonical request
      // before every renderer, Studio, print, and PDF path consumes it.
      const activeSpec = structuredClone(
        mode === "quick"
          ? canonicalQuickCreateRequest({ ...plannedSpec, prompt: requestSpec.prompt })
          : plannedSpec,
      );
      setSpec(activeSpec);
      ...
      const plannedProject = applyAIWorksheetContent(structured, activeSpec);
      const finalProject = profile
        ? { ...plannedProject, visualDirection: profile.visualDirection }
        : plannedProject;
      setDoc(document);
      ...
      const unsupported = failure?.details?.filter((detail) =>
        detail.startsWith("Unsupported prompt requirement: "),
      );
      setGenerationError(
        failure instanceof UnsupportedAdvancedActivityTypeError
          ? failure.message
          : unsupported?.length
          ? `I can’t generate this request exactly yet: ${unsupported
              .map((detail) => detail.replace("Unsupported prompt requirement: ", ""))
              .join(" · ")}. Please change that part of the prompt.`
            ...
            {(
              ...
              <button
                ...
                onClick={() => {
                  setMode(m.id);
                  if (m.id === "quick") {
                    setSpec((current) => canonicalQuickCreateRequest(current));
                  }
                }}
                className={cn(
