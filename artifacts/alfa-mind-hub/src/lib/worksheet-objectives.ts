...
const profiles: Record<WorksheetMechanicId, Omit<ObjectiveProfile, "variant">> = {
  ...
  "maze-route": {
    mechanic: "maze-route",
    activityLabel: "Maze",
    objective: "Find a continuous route from START to FINISH through a maze.",
    kinds: ["maze"],
  },
  "count-match": {
...
const byPrompt: Array<{ test: RegExp; mechanic: WorksheetMechanicId }> = [
  ...
  {
    test: /(?=.*\b(?:numbers?|numerals?|digits?)\b)(?=.*\b(?:trace|tracing|handwriting|write|writing)\b)/i,
    mechanic: "number-write",
  },
  {
...
export function promptRequestsMechanic(spec: WorksheetSpec): WorksheetMechanicId | undefined {
  const activityTypeMechanic = promptActivityMechanicFor(spec);
  if (activityTypeMechanic) return activityTypeMechanic;
  ...
  return matched.size === 1 ? rendererMechanicFor([...matched][0]!) : undefined;
}
...
export function resolveObjectiveProfile(spec: WorksheetSpec): ObjectiveProfile {
  ...
  // A teacher-selected Advanced activity is stronger than inferred wording or
  // a generic skill. It either resolves to the one truthful mechanic below or
  // is rejected by the generation entry gate before this function is reached.
  let mechanic: WorksheetMechanicId | undefined =
    advancedActivityMechanicFor(spec) ?? promptActivityMechanicFor(spec);

  // 1. structured idea ids — the strongest signal
  if (!mechanic && spec.objectiveId && byObjectiveId[spec.objectiveId]) {
  ...
  if (!mechanic) {
    ...
    else if (/match|connect|puzzle|line/.test(t))
      mechanic = /\bshapes?\b/.test(t) ? "match-pairs" : "count-match";
  ...
  const domain = structured
    ...
    : domainForSpec(spec);
  let resolved = rendererMechanicFor(mechanic ?? nearestMechanicForDomain(domain));
  if (
    !structured &&
    !isAdvancedActivityTypeLocked(spec) &&
    !isPromptActivityTypeLocked(spec) &&
    !crossDomainMechanics.includes(resolved) &&
    !mechanicsByDomain[domain].includes(resolved)
  ) {
    resolved = rendererMechanicFor(mechanicsByDomain[domain][0]!);
  }
...
export function profileForMechanic(
  ...
  mechanic = rendererMechanicFor(mechanic);
  const text = `${spec.prompt ?? ""} ${spec.skill ?? ""} ${spec.activityType ?? ""}`;
