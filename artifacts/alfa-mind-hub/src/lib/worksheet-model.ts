...
import type { CanonicalArtworkManifest, WorksheetArtworkManifest } from "./artwork-contract";
import type { PromptRequirementContract } from "./creator-options";
import {
  applyPrintMode,
...
export type RenderedCountObject = {
  ...
  letter?: string;
  /** Versioned local-vector recipe shared by preview, saved projects and PDF. */
  artwork?: CanonicalArtworkManifest;
};
...
export type WorksheetMechanicId =
  ...
  | "cut-create-scene"
  | "cut-create-count"
  /** Follow a route from START to FINISH through continuous maze walls. */
  | "maze-route";
...
export type MatchActivity = {
  ...
  /** explicit prompt placement for the shuffled number bank */
  numberBankSide?: "left" | "right";
};
...
/**
 * A genuine paper maze. The model owns every wall and the verified solution;
 * the renderer merely draws this geometry and never invents a route.
 */
export type MazeActivity = {
  kind: "maze";
  mechanic: "maze-route";
  rows: number;
  columns: number;
  ...
  finish: { row: number; column: number };
  /** ordered grid cells from START through FINISH, for the teacher answer key */
  solution: Array<{ row: number; column: number }>;
  /**
   * Optional local topic cue. It is rendered beside the maze, never inside the
   * wall graph, so decoration cannot obscure a playable route.
   */
  decoration?: RenderedCountObject & {
    label: string;
  };
};

export function mazeIntegrityIssues(activity: MazeActivity): string[] {
  const issues: string[] = [];
  const key = (row: number, column: number) => `${row}:${column}`;
  const cells = new Map(activity.cells.map((cell) => [key(cell.row, cell.column), cell]));
  if (activity.rows < 4 || activity.columns < 4) issues.push("Maze grid is too small.");
  if (cells.size !== activity.rows * activity.columns) issues.push("Maze does not contain every grid cell.");
  const at = (row: number, column: number) => cells.get(key(row, column));
  const inside = (row: number, column: number) =>
    row >= 0 && row < activity.rows && column >= 0 && column < activity.columns;
  for (const cell of cells.values()) {
    if (!inside(cell.row, cell.column)) issues.push("Maze includes a cell outside its grid.");
    const neighbors = [
      [cell.row - 1, cell.column, "top", "bottom"],
      [cell.row, cell.column + 1, "right", "left"],
      [cell.row + 1, cell.column, "bottom", "top"],
      [cell.row, cell.column - 1, "left", "right"],
    ] as const;
    for (const [row, column, ownWall, neighborWall] of neighbors) {
      const neighbor = at(row, column);
      if (!neighbor) {
        if (!cell[ownWall]) issues.push("Maze boundary has an opening.");
      } else if (cell[ownWall] !== neighbor[neighborWall]) {
        issues.push("Adjacent maze cells disagree about a wall.");
      }
    }
  }
  const start = at(activity.start.row, activity.start.column);
  const finish = at(activity.finish.row, activity.finish.column);
  if (!start || !finish) issues.push("Maze START or FINISH is outside its grid.");
  if (!activity.solution.length) issues.push("Maze has no answer route.");
  if (
    activity.solution[0]?.row !== activity.start.row ||
    activity.solution[0]?.column !== activity.start.column ||
    activity.solution.at(-1)?.row !== activity.finish.row ||
    activity.solution.at(-1)?.column !== activity.finish.column
  ) {
    issues.push("Maze answer route does not run from START to FINISH.");
  }
  for (let index = 1; index < activity.solution.length; index++) {
    const from = activity.solution[index - 1]!;
    const to = activity.solution[index]!;
    const cell = at(from.row, from.column);
    const dx = to.column - from.column;
    const dy = to.row - from.row;
    const wall = dx === 1 ? "right" : dx === -1 ? "left" : dy === 1 ? "bottom" : dy === -1 ? "top" : undefined;
    if (!cell || !wall || cell[wall]) issues.push("Maze answer route crosses a wall or jumps cells.");
  }
  const hasDeadEnd = [...cells.values()].some((cell) => {
    if (
      (cell.row === activity.start.row && cell.column === activity.start.column) ||
      (cell.row === activity.finish.row && cell.column === activity.finish.column)
    ) {
      return false;
    }
    return [cell.top, cell.right, cell.bottom, cell.left].filter((wall) => !wall).length === 1;
  });
  if (!hasDeadEnd) issues.push("Maze has no wrong-path dead end.");
...
export type MemoryCard = Pick<RenderedCountObject, "id" | "asset" | "character" | "artwork"> & {
...
export type PageSemanticRequirements = {
  ...
  /** exact number of countable groups the teacher asked a counting page to show */
  requiredGroupCount?: number;
  /** exact number of choices printed in each row */
  requiredChoiceCount?: number;
  /** Ordered object/quantity pairs for a detailed counting request. */
  requiredCountGroups?: Array<{ asset: VisualAssetKey; count: number }>;
  /** every quantity an explicit counting range requires, e.g. 1–5 */
  requiredQuantityRange?: [number, number];
  /** explicit placement for a count-and-match number bank */
  numberBankSide?: "left" | "right";
...
export type ImmutableGenerationSpecification = {
  ...
  /** Parsed explicit teacher requirements frozen with the generated pack. */
...
export type WorksheetProject = {
  ...
  /**
   * Optional teacher-selected palette. It recolors the renderer while keeping
   * the chosen visual direction's drawing language and subject artwork.
   */
  ...
  /** Canonical index of all rendered local artwork recipes in this project. */
  artworkManifest?: WorksheetArtworkManifest;
  teacherNotes: string[];
  /**
