...
import { visualAssetKeys, visualObjects } from "./semantic-topics";
import { basicShapes, shapePairs } from "./object-semantics";
import { resolveSubject } from "./worksheet-subjects";
import { localIllustrationAssetsForSpec, localMazeDecorationAssetsForSpec } from "./illustration-library";
import { resolveVisualAssetMentions, visualAssetLabel } from "./visual-asset-library";
...
function explicitMatchingColumns(spec: WorksheetSpec) {
  const columnAssets = (side: "left" | "right") => {
    const match = new RegExp(
      String.raw`\b(?:on\s+)?(?:the\s+)?${side}\s*:\s*([^.!?]+)`,
      "i",
    ).exec(spec.prompt ?? "");
    if (!match?.[1]) return [];
    const candidates = resolveVisualAssetMentions(match[1]);
    // Keep the most specific phrase at an overlapping position: "fish bowl"
    // must be one matching item, never a fish plus a bowl.
    return candidates
      .filter(
        (candidate, index) =>
          !candidates
            .slice(0, index)
            .some((accepted) => candidate.start < accepted.end && accepted.start < candidate.end),
      )
      .map((candidate) => ({
        asset: candidate.asset,
        // "yarn ball" is a specific teacher-facing label for the existing ball
        // illustration; retain it on the printed matching card.
        label: visualAssetLabel(candidate),
      }));
  };
  const left = columnAssets("left");
  const right = columnAssets("right");
  return left.length >= 2 && left.length === right.length ? { left, right } : undefined;
}
...
export function buildFindTargetPage(ctx: BuildContext): WorksheetPageModel {
  ...
  const targetCount = tokens.visualScaffolding === "high" ? 2 : tokens.visualScaffolding === "medium" ? 3 : 4;
  const distractorItems = Array.from(
    { length: tokens.distractorCount },
    (_, index) => distractors[index % Math.max(1, distractors.length)] ?? target,
  );
  const assets = shuffle([...Array(targetCount).fill(target), ...distractorItems], next);
  ...
  return {
    ...base(ctx, {
      ...
      instruction: `Find and circle every ${subject.singular}.`,
    }),
    activityType: "Find",
    layout: "stacked-rows",
...
export function buildMatchPairsPage(ctx: BuildContext): WorksheetPageModel {
  ...
  const explicitColumns = explicitMatchingColumns(ctx.spec);
  if (explicitColumns) {
    const left = explicitColumns.left.map(({ asset, label }, index) => ({
      id: `match-left-${index + 1}`,
      pairId: `pair-${index + 1}`,
      asset,
      label,
    }));
    const right = explicitColumns.right.map(({ asset, label }, index) => ({
    ...
    return {
      ...base(ctx, {
        title: "Animal Friends & Their Things",
        instruction: "Draw one line from each animal to the thing that belongs with it.",
      }),
      ...
      activity: {
        kind: "match-pairs",
        mechanic: "match-pairs",
        subtype: "identical-pairs",
        relationship: "animal-to-item",
        left,
        right,
      },
      contentLocked: true,
      answerKey: left.map((item) => {
        ...
        return {
          groupId: item.id,
          answer: partner ? right.indexOf(partner) + 1 : 0,
          answerText: partner?.label ?? "",
        };
      }),
    } as WorksheetPageModel;
  }
  const babyParent = ctx.semanticRequirements?.activitySubtype === "baby-parent";
  const matchingAssets = subject.locked
    ? subject.assets
    : shuffle(localIllustrationAssetsForSpec(ctx.spec, "match-pairs"), next);
  const pairs: Array<{
    baby: VisualAssetKey;
          ...
          length: Math.max(
            ...
            Math.min(
              ...
              ctx.semanticRequirements?.requiredItemCount
                ? Math.ceil(ctx.semanticRequirements.requiredItemCount / 2)
                : Math.min(4, matchingAssets.length),
            ),
          ),
        },
        (_unused, index) => {
          const asset = matchingAssets[index % matchingAssets.length]!;
          return {
            baby: asset,
            parent: asset,
  ...
  return {
    ...
    // A named object is part of the matching question, not decorative theme
    // art. Keep it through final theme/pedagogy repair so a requested turtle
    // cannot disappear before the prompt-fidelity gate.
    ...(subject.locked ? { contentLocked: true } : {}),
...
function mazeForSeed(rows: number, columns: number, seed: number) {
  const key = (row: number, column: number) => `${row}:${column}`;
  const next = rng(seed);
  const cells = new Map<string, MazeCell & { visited: boolean }>();
  for (let row = 0; row < rows; row++) {
    for (let column = 0; column < columns; column++) {
      cells.set(key(row, column), { row, column, top: true, right: true, bottom: true, left: true, visited: false });
    }
  }
  const directions = [
    { dr: -1, dc: 0, own: "top", other: "bottom" },
    { dr: 0, dc: 1, own: "right", other: "left" },
    { dr: 1, dc: 0, own: "bottom", other: "top" },
    { dr: 0, dc: -1, own: "left", other: "right" },
  ] as const;
  const stack = [cells.get(key(0, 0))!];
  ...
  while (stack.length) {
    ...
    const choices = shuffle(
      directions
        .map((direction) => ({
          direction,
          cell: cells.get(key(current.row + direction.dr, current.column + direction.dc)),
        }))
        .filter((choice): choice is { direction: (typeof directions)[number]; cell: MazeCell & { visited: boolean } } => Boolean(choice.cell && !choice.cell.visited)),
      next,
    );
    ...
    current[choice.direction.own] = false;
    choice.cell[choice.direction.other] = false;
    choice.cell.visited = true;
    stack.push(choice.cell);
  }
  const plainCells = [...cells.values()].map(({ visited: _visited, ...cell }) => cell);
  const byCell = new Map(plainCells.map((cell) => [key(cell.row, cell.column), cell]));
  const start = { row: 0, column: 0 };
  const finish = { row: rows - 1, column: columns - 1 };
  const route = new Map<string, string | undefined>([[key(start.row, start.column), undefined]]);
  const queue = [start];
  while (queue.length) {
    const current = queue.shift()!;
    if (current.row === finish.row && current.column === finish.column) break;
    const cell = byCell.get(key(current.row, current.column))!;
    for (const direction of directions) {
      if (cell[direction.own]) continue;
      const candidate = { row: current.row + direction.dr, column: current.column + direction.dc };
      const candidateKey = key(candidate.row, candidate.column);
      if (!byCell.has(candidateKey) || route.has(candidateKey)) continue;
      route.set(candidateKey, key(current.row, current.column));
      queue.push(candidate);
    }
  }
  const solution: Array<{ row: number; column: number }> = [];
  let cursor: string | undefined = key(finish.row, finish.column);
  while (cursor) {
    const [row, column] = cursor.split(":").map(Number);
    solution.push({ row, column });
    cursor = route.get(cursor);
  }
  return { rows, columns, cells: plainCells, start, finish, solution: solution.reverse() };
}

/**
 * Generate a connected, solvable maze with at least one wrong-path dead end.
 * Several deterministic attempts make the dead-end guarantee explicit rather
 * than relying on the usual behaviour of a random depth-first maze.
 */
export function buildMazePage(ctx: BuildContext): WorksheetPageModel {
  const tokens = resolveAgeTokens(ctx.spec.level);
  const subject = resolveSubject(ctx.spec);
  const themeLabel =
    ctx.spec.theme.trim() && ctx.spec.theme.trim().toLowerCase() !== "insects"
      ? ctx.spec.theme.trim()
      : undefined;
  const contextLabel = themeLabel ?? (subject.kind === "object" ? subject.label : undefined);
  const rows = tokens.visualScaffolding === "high" ? 6 : tokens.visualScaffolding === "medium" ? 7 : 8;
  const columns = tokens.visualScaffolding === "high" ? 6 : tokens.visualScaffolding === "medium" ? 8 : 9;
  let maze = mazeForSeed(rows, columns, ctx.seed + 9401);
  for (let attempt = 1; attempt < 18 && mazeIntegrityIssues({ ...maze, kind: "maze", mechanic: "maze-route" }).length; attempt++) {
    maze = mazeForSeed(rows, columns, ctx.seed + 9401 + attempt * 7919);
  }
  const activity = { kind: "maze" as const, mechanic: "maze-route" as const, ...maze };
  const decorationAssets = localMazeDecorationAssetsForSpec(ctx.spec);
  const decorationAsset = decorationAssets[Math.abs(ctx.seed) % Math.max(1, decorationAssets.length)];
  const activityWithDecoration = decorationAsset
    ? {
        ...activity,
        decoration: {
          id: "maze-decoration",
    ...
    : activity;
  const issues = mazeIntegrityIssues(activityWithDecoration);
  if (issues.length) throw new Error(`Could not construct a valid maze: ${issues.join(" ")}`);
  return {
    ...base(ctx, {
      title: contextLabel ? `${contextLabel} Maze: Find the Finish` : "Maze: Find the Finish",
      instruction: "Start at START. Trace one path through the maze to FINISH.",
    }),
    activityType: "Maze",
    layout: "stacked-rows",
    activity: activityWithDecoration,
    answerKey: [
      {
        groupId: "maze-route",
        answer: activity.solution.length,
        answerText: `START → FINISH (${activity.solution.length} cells)`,
      },
    ],
    contentLocked: true,
    footerNote: "Try a new path when you reach a dead end.",
  };
}
...
export function buildComparePage(ctx: BuildContext): WorksheetPageModel {
  ...
  const tokens = resolveAgeTokens(ctx.spec.level);
  const subject = resolveSubject(ctx.spec);
  const assets = subject.locked ? subject.assets : shuffle(localIllustrationAssetsForSpec(ctx.spec, "compare-quantity"), next);
...
export function buildSizePage(ctx: BuildContext): WorksheetPageModel {
  ...
  const tokens = resolveAgeTokens(ctx.spec.level);
  const subject = resolveSubject(ctx.spec);
  const assets = subject.locked ? subject.assets : shuffle(localIllustrationAssetsForSpec(ctx.spec, "compare-size"), next);
  const rowCount = Math.max(3, Math.min(4, tokens.itemsPerPage - 1));
...
function requestedPatternCount(
  ...
  const match =
    /(\d{1,2}|one|two|three|four|five|six|seven|eight)\s+(?:(?:[a-z]{2,12}|[A-Z]{2,3})\s+){0,5}(patterns?|rows?|sequences?)\b/i.exec(
...
export function buildSequencePage(ctx: BuildContext): WorksheetPageModel {
  ...
  const tokens = resolveAgeTokens(ctx.spec.level);
  const subject = resolveSubject(ctx.spec);
  const assets = subject.locked ? subject.assets : shuffle(localIllustrationAssetsForSpec(ctx.spec, "find-and-count"), next);
  const [min, max] = ctx.range;
