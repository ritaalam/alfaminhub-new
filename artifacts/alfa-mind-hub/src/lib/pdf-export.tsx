...
import { paperSizes, type PaperFormat, type RenderMode, type WorksheetProject } from "@/lib/worksheet-model";

/**
 * Real PDF export.
 *
 * The Studio already renders every worksheet page as a millimetre-accurate
 * paper sheet, so the PDF is produced from those exact same components: each
 * page is mounted offscreen at 1:1 scale, rasterised at high resolution and
 * placed on the project's selected paper format. Page order, artwork, text and
 * the selected print mode/palette are therefore identical to the Studio view.
 *
...
export function pdfPageGeometry(paper: PaperFormat) {
  const size = paperSizes[paper] ?? paperSizes.A4;
  return { widthMm: size.w, heightMm: size.h, format: [size.w, size.h] as [number, number] };
}
...
export async function exportProjectToPdf(
  ...
  const geometry = pdfPageGeometry(project.meta.paper);
  ...
  try {
    ...
    for (let i = 0; i < nodes.length; i += 1) {
      ...
      const canvas = await html2canvas(sheet, {
        ...
        useCORS: true,
        logging: false,
        windowWidth: Math.ceil(geometry.widthMm / MM_PER_PX),
      });
      const image = canvas.toDataURL("image/jpeg", 0.94);
      if (i > 0) pdf.addPage(geometry.format, "portrait");
      pdf.addImage(
        image,
        "JPEG",
        0,
